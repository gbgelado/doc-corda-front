...
import com.corda.samples.cordasample.contract.DebtContract
...
* flow start CreateDebtFlow name: "Joao", document: "123.123.123-12"
*
* Consultando os estados criados:
* run vaultQuery contractStateType: tech.bluchain.sample.state.DebtState
*
* */
...
object CreateDebt {
    ...
    class CreateDebtFlow(
        ...
        override fun call(): SignedTransaction {
            ...
            val indexDocument = DebtSchemaV1.PersistentDebt::document.equal(document)
            val criteria = QueryCriteria.VaultCustomQueryCriteria(expression = indexDocument)
            val DebtList = serviceHub.vaultService.queryBy<DebtState>(criteria).states
            ...
            */
            ...
            val Debt = DebtModel(Instant.now(), ourIdentity, creditor, debtAmount, comment)

            // criando um estado de pessoa
            val DebtState = DebtState(Debt)

            // criando o command para validação do contract
            val txCommand = Command(
                DebtContract.Commands.CreateDebt(),
                DebtState.participants.map { it.owningKey }
            )
            ...
            val txBuilder = TransactionBuilder(notary)
                ...
                .addOutputState(DebtState, DebtContract::class.java.canonicalName)
    ...
    * */
    @InitiatedBy(CreateDebtFlow::class)
